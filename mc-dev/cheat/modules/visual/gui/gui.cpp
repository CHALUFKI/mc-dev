...

		{
			...
			if (sizeof(activerenderinfo::viewport) > 0 && sizeof(activerenderinfo::projection) > 0 && sizeof(activerenderinfo::modelview) > 0)
			{
				...
				if (gui::draw_hud)
				{
					...
					struct tm timeinfo;  // Remove the asterisk - store directly, not as pointer
					localtime_s(&timeinfo, &now);  // Use localtime_s instead

					char buffer[80];
					strftime(buffer, sizeof(buffer), "%H:%M:%S", &timeinfo);  // Pass address with &
